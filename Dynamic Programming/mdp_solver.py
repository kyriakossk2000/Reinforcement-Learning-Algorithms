...
class ValueIteration(MDPSolver):
    ...
    def _calc_value_func(self, theta: float) -> np.ndarray:
        ...
        **DO NOT ALTER THE MDP HERE**
        ...
        ### PUT YOUR CODE HERE ###
        while True:  # loop until max difference is less than theta
            delta = 0
            for state in range(self.state_dim):
                value = V[state]
                action_V = np.zeros(self.action_dim)
                for action in range(self.action_dim):
                    action_value = 0
                    for following_state in range(self.state_dim):
                        r = self.mdp.R[state, action, following_state]
                        p = self.mdp.P[state, action, following_state]
                        action_value = action_value + p * (r + self.gamma * V[following_state])
                    action_V[action] = action_value
                V[state] = np.max(action_V)
                value_diff = abs(value - V[state]) # calc the max diff between the old and new value func
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        for state in range(self.state_dim):
            ...
            for action in range(self.action_dim):
                action_value = 0
                for following_state in range(self.state_dim):
                    # calc Q action value for each action over all states
                    r = self.mdp.R[state, action, following_state]
                    p = self.mdp.P[state, action, following_state]
                    action_value = action_value + p * (r + self.gamma * V[following_state])
                action_V[action] = action_value
                best_action_value = np.argmax(action_V)
                policy[state, best_action_value] = 1.0  # deterministic policy
...
class PolicyIteration(MDPSolver):
    ...
    def _policy_eval(self, policy: np.ndarray) -> np.ndarray:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        while True:
            delta = 0
            for state in range(self.state_dim):
                action_value = V[state]
                action = np.argmax(policy[state, :])
                update_V = 0
                for following_state in range(self.state_dim):
                    p = self.mdp.P[state, action, following_state]
                    r = self.mdp.R[state, action, following_state]
                    update_V += policy[state, action] * p * (r + self.gamma * V[following_state])
                V[state] = update_V
                value_diff = abs(action_value - V[state]) # calc the max diff between the old and new value func
                delta = max(delta, value_diff)
    ...
    def _policy_improvement(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        ### PUT YOUR CODE HERE ###
        while True:
            V = self._policy_eval(policy)  # policy eval

            updated_policy = np.zeros((self.state_dim, self.action_dim))

            for state in range(self.state_dim):
                action_V = np.zeros(self.action_dim)
                for action in range(self.action_dim):
                    p = self.mdp.P[state, action, :]
                    r = self.mdp.R[state, action, :]
                    action_V[action] = np.sum(p * (r + self.gamma * V))
                best_action = np.argmax(action_V)   # best action chooses for the state that maximizes value
                updated_policy[state, best_action] = 1.0

            if (policy == updated_policy).all():    # check if policy same with updated policy to see if policy has converged
                break
            else:  # if policy didn't converge continue
